...
/**
 * Class ReporteDos, hereda de la clase Thread
 * 
...
public class ReporteDos extends Thread {
		...
		if (security == true) {
			...
			try {
				...
				documento.add(new Paragraph(
						"		            Consulta de los albums creados en la fecha : " + nombrea, fuente));// Almaceno
																												// informacion
				documento.add(new Paragraph(" "));// Almaceno informacion
				documento.add(new Paragraph(" "));// Almaceno informacion
				...
				documento.add(new Paragraph("       "));
				documento.add(new Paragraph("       "));
				PdfPTable tabla = new PdfPTable(6);// Crea tablas y por medio del construcctor le mando el numero de
				...
				while (rs.next()) {
					tabla.addCell("    " + contador);// Recibir una cadena de texto, deben completarse la cantidad de
														// celdas como tantas cantidad de columnas, si no no se agrega
			...
			try {
				...
				documento.add(new Paragraph("       "));
				documento.add(new Paragraph(
						"  Contraseña de disquera incorrecta no puedes visualizar reportes oficiales sin porporcionar la contraseña correcta           "));
				documento.add(new Paragraph("       "));
				documento.add(new Paragraph("					Cualquier duda o aclaraciòn favor de marcar al numero "
						+ "5537015892 o bien 5510727166 y enseguida se le atenderà."));
				Image imgTres = Image.getInstance("src\\img\\abajo.png");
	...
	/**
	 * Metodo para consultar nombre de la disquera
	 * 
	 * @return dato
	 */
	...
	/**
	 * Metodo para consultar nombre del artista
	 * 
	 * @return dato
	 */
